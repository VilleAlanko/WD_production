...
for k in range(num_members):
    for i in range(int(N_data / 2) - 0):
        for j in range(int(N_data / 2) - 0):
            chi_k_squared[k] += (y_eta_lept[k, i] - y_exp_eta_lept[i]) * C_inverse_eta_lept[i, j] * (y_eta_lept[k, j] - y_exp_eta_lept[j])
            #print((y_eta_lept[k, i] - y_exp_eta_lept[i]) * C_inverse_eta_lept[i, j] * (y_eta_lept[k, j] - y_exp_eta_lept[j]), k, i, j)
    for i in range(int(N_data / 2) - 0):
        for j in range(int(N_data / 2) - 0):
            chi_k_squared[k] += (y_pTD[k, i] - y_exp_pTD[i]) * C_inverse_pTD[i, j] * (y_pTD[k, j] - y_exp_pTD[j])
...
print("Neff =", np.exp(1. / (num_members * 1.) * np.sum(omega_k_chi_squared * np.log((num_members * 1.) / (omega_k_chi_squared)))))

np.savetxt('output/omega_' + PDF_set + '_' + which_cross_sections_included + '.txt', omega_k_chi_squared)
