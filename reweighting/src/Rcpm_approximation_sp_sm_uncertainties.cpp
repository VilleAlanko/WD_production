...
int OLD(string PDF_set, int num_err_members)
{
    ...
    for (int member_id = 0; member_id < num_err_members + 1; member_id++)
    {
        ...
        for (int x_index = 0; x_index < N; x_index++)
        {
            ...
            if (member_id != 0)
            {
                err_member_vals[member_id - 1][x_index] = 1. - 2. * (epsilon * (d - dbar) + s - sbar) / (s + sbar);
            }
            else
            {
                best_vals[x_index] = 1. - 2. * (epsilon * (d - dbar) + s - sbar) / (s + sbar);
            }
        }
    }
    ...
    string filename = "output/Rcpm_approximation_sp_sm_errors/" + PDF_set + "_OLD_plus.txt";
    ...
    filename = "output/Rcpm_approximation_sp_sm_errors/" + PDF_set + "_OLD_minus.txt";
    ...
    filename = "output/Rcpm_approximation_sp_sm_errors/" + PDF_set + "_OLD_mem_vals.txt";
